...
describe("Assertions", () => {
    ...
    it("Implicit Assertions", () => {
      ...
      // it can get 2, or 3 arguments, and we can validate if element has specific attribute, 
      //and the value of it
       ...
       * 2. Click on Add Product Button > Modal Opens
       * 3. Close the modal and validate modal is not visible
       */
    ...
    it("Explicit Assertions", () => {
      ...
      /**
       * NOTE: invoke() or without invoke is preference.
       * Has no performance difference or benefit one over the another.
       *
    ...
    const expected = [ 'Apple', 'Microsoft', 'Tesla' ];

    cy.get('#checkbox-button-group div').each(($el, index) => {
        cy.wrap($el).find('label').should('have.text', expected[index]);
 ...
 const options = ['Microsoft', 'Apple']
  ...
  cy.get('[id^="company_dropdown"]').each(($el, index) => {
    ...
    .should('have.value', options[index])
